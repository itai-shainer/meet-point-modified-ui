...
export default function RouteHistory() {
  ...
  const toggleFavorite = async (e, item) => {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ['routeHistory'] });
    } catch (error) {
  ...
  const driverOriginLatLng = useMemo(() => {
    if (!selectedHistory || !window.google) return null;
    return new window.google.maps.LatLng(
      selectedHistory.driver_origin_lat,
      selectedHistory.driver_origin_lng
    );
  }, [selectedHistory]);
  ...
  const passengerOriginLatLng = useMemo(() => {
    if (!selectedHistory || !window.google) return null;
    return new window.google.maps.LatLng(
      selectedHistory.passenger_origin_lat,
      selectedHistory.passenger_origin_lng
    );
  }, [selectedHistory]);
  ...
  const destinationLatLng = useMemo(() => {
    if (!selectedHistory || !window.google) return null;
    return new window.google.maps.LatLng(
      selectedHistory.destination_lat,
      selectedHistory.destination_lng
    );
  }, [selectedHistory]);
        ...
        <div className="max-w-7xl mx-auto p-4 md:p-8">
          ...
          <div className={`mb-4 p-3 rounded-lg ${darkMode ? 'bg-gray-800/30' : 'bg-white/60'}`}>
            <div className={`flex items-center gap-2 text-sm ${darkMode ? 'text-gray-400' : 'text-gray-700'}`}>
              <Calendar className="w-4 h-4" />
              <span className="font-medium">{format(new Date(selectedHistory.created_date), 'dd MMMM yyyy, HH:mm')}</span>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="space-y-4">
              <JourneyDetails 
                plan={plan}
                originAddress={selectedHistory.driver_origin_address}
                passengerAddress={selectedHistory.passenger_origin_address}
                destinationAddress={selectedHistory.destination_address}
                darkMode={darkMode}
                isDirectPickup={selectedHistory.is_direct_pickup}
              />
            </div>
            ...
            <div className="lg:sticky lg:top-4 h-fit">
              <MapView
                ...
                highlightedIndex={-1}
                transitRoutePolyline={plan?.passenger_arrivals?.find(p => p.mode === 'transit')?.transit_route_polyline}
              />
            </div>
          </div>
        </div>
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 relative ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-8 relative z-10">
        {/* Glass navbar */}
        <div className={`flex flex-row justify-between items-center gap-4 mb-8 p-4 rounded-2xl backdrop-blur-md border shadow-xl ${darkMode ? 'bg-gray-900/90 border-white/10' : 'bg-white/90 border-white/20'}`}>
        ...
        {isLoading ? (
          <div className="flex items-center justify-center py-20">
            <div className="flex flex-col items-center gap-3">
              ...
              <p className={`text-lg ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>טוען היסטוריה...</p>
            </div>
          </div>
          ...
          <div className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white'} rounded-2xl shadow-xl p-12 border-2 text-center`}>
            <div className={`inline-flex p-6 rounded-full mb-4 ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
              <MapPin className={`w-12 h-12 ${darkMode ? 'text-gray-400' : 'text-gray-400'}`} />
            </div>
